...
class RewriteHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        query_suffix = ('?' + parts.query) if parts.query else ''

        # Pre-rendered static page wins over the JS template, if it exists.
        # Generated by `node _imgtools/prerender.cjs` at <route>/index.html.
        # Mirrors nginx's try_files $uri $uri/ behavior in local dev.
        if path:
            prerendered = os.path.join(os.getcwd(), path.lstrip('/'), 'index.html')
            if os.path.isfile(prerendered):
                self.path = path + '/index.html' + query_suffix
                return super().do_GET()

        # Clean URLs that resolve to the solution template
        if path in EXPLICIT_ROUTES:
            self.path = '/solution-template.html' + query_suffix
        # Pretty URLs for static pages
        elif path in PRETTY_PAGES:
            self.path = PRETTY_PAGES[path] + query_suffix
